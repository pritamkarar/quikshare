...
describe('LandingScreen', () => {
   ...
   * both times — one label per intent, so "Create a session" is never
   * competing with a "Start sharing" that means the same thing.
   */
  it('offers both ways in, in the hero and again at the foot', () => {
    render(<LandingScreen />);
    expect(screen.getAllByRole('button', { name: /create a session/i })).toHaveLength(2);
  ...
  it('routes to /new rather than creating in place', async () => {
    ...
    render(<LandingScreen />);
    await userEvent.click(screen.getAllByRole('button', { name: /create a session/i })[0]!);
